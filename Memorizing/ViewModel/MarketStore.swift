...
class MarketStore: ObservableObject {
    ...
    @Published var sendWordNote: WordNote = WordNote(id: "", noteName: "", noteCategory: "", enrollmentUser: "", repeatCount: 0, notePrice: 0)
    
    // MARK: - 마켓의 전체 단어장들을 fetch 하는 함수
    func fetchMarketWordNotes() {
            ...
            .getDocuments { (snapshot, error) in
                ...
                if let snapshot {
                    for document in snapshot.documents {
                        ...
                        let marketWordNote: WordNote = WordNote(id: id, noteName: noteName, noteCategory: noteCategory, enrollmentUser: enrollmentUser, repeatCount: repeatCount, notePrice: notePrice)
    ...
    // MARK: - 단어장을 들어가면 해당 단어장의 단어들을 fetch 하는 함수
    func fetchWords(wordNoteId: String) {
            ...
            .getDocuments { (snapshot, error) in
                ...
                if let snapshot {
                    for document in snapshot.documents {
                        ...
                        let word: Word = Word(id: id, wordString: wordString, wordMeaning: wordMeaning, wordLevel: wordLevel)
    ...
    // MARK: - 마켓에 단어장을 등록하면 단어장과 단어들이 마켓 DB에 올라가게 하는 함수
    func addMarketWordNote(noteId: String, userId: String, notePrice: Int) {
        let noteId = noteId
