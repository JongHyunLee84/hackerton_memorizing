        ...
        VStack {
            ...
            HStack {
                ...
                RoundedRectangle(cornerRadius: 5)
                    ...
                    .overlay {
                        HStack {
                            TextField("포인트 입력", text: $input.value)
                                .padding(.leading)
                                .font(.footnote)
            ...
            Button(action: {
                if selectedNote != "" {
                    marketStore.addMarketWordNote(noteId: selectedNote, userId: userStore.user?.id ?? "", notePrice: Int(input.value) ?? 0)
                    
                    dismiss()
                }
            }, label: {
                RoundedRectangle(cornerRadius: 20)
                    .fill(selectedNote == "" ? Color.gray2 : Color.mainBlue)
