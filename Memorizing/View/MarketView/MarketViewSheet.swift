...
struct MarketViewSheet: View {
    ...
    var body: some View {
            ...
            if userStore.myWordNoteIdArray.contains(wordNote.id) {
                ...
                .alert(isPresented: $isAlertToggle) {
                    if isCoinCheckToggle {
                        return Alert(title: Text("구매하기"), message: Text("\(wordNote.noteName)을(를) 구매하시겠습니까?"), primaryButton: .destructive(Text("구매하기"), action: {
                            userStore.currentUserCoinCheck(marketWordNote: wordNote, words: marketStore.words)
                            
                            dismiss()
                        }), secondaryButton: .cancel(Text("취소")))
                    } else {
                        return Alert(title: Text("포인트 부족"), message: Text("포인트가 부족합니다"),
        ...
        .onAppear{
            marketStore.fetchWords(wordNoteId: wordNote.id)
            userStore.fetchMyWordNoteArray()
        }
        .onDisappear{
