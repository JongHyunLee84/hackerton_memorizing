...
struct AddWordView: View {
    ...
    @Binding var list : [Word]
    // MARK: - 단어, 문장, 질문과 답 피커 만들기 -> 아래 Enum으로 유형 선언되어 있음
    @State private var segmnetationSelection : AddWordCategory = .단어
                ...
                HStack(spacing: 30) {
                    ...
                    HStack {
                        ...
                        .alert(isPresented: $showingAlert) {
                            Alert(title: Text("암기장을 저장하시겠습니까?"), message: Text("취소시 저장된 데이터는 사라집니다"), primaryButton: .destructive(Text("취소하기"), action: {
                            }), secondaryButton: .cancel(Text("저장하기"), action: {
                                userStore.fetchMyWords(wordNote: wordNote){
                                    self.list = userStore.myWords
                                }
                                dismiss()
                            }))
                        }
            ...
            Section {
                ScrollView(showsIndicators: false) {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        //MARK: - 2. 문장 암기장 만들기
                        else if segmnetationSelection == .문장 {
            ...
            Section {
                VStack {
                    HStack {
                        ...
                        Button {
                            userStore.addMyWord(wordNote: wordNote, word: Word(id: UUID().uuidString, wordString: wordString, wordMeaning: wordMeaning, wordLevel: wordLevel))
                            wordString = ""
                            wordMeaning = ""
                            wordLevel = 0
...
//        AddWordView(wordNote: WordNote(id: "", noteName: "", noteCategory: "", enrollmentUser: "", repeatCount: 0, notePrice: 0), list: [Word(id: "", wordString: "", wordMeaning: "", wordLevel: 0)])
//    }
//}
