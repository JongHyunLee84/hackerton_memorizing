...
//    @State private var currentWordDef: String
    ...
    var body: some View {
        VStack {
            ...
            // MARK: 카드뷰
            ZStack {
                wordCardFrontView(listLength: word.count, currentListLength: $num, currentWord: word[num].wordString, degree: $frontDegree)
                wordCardBackView(listLength: word.count, currentListLength: $num, currentWordDef: word[num].wordMeaning, degree: $backDegree)
            }
            .onTapGesture {
                flipCard()
            }
            ...
            LevelCheck(isShowingModal: .constant(false), isFlipped: $isFlipped, isDismiss: $isDismiss, totalScore: $totalScore, lastWordIndex: wordCount, num: $num, isShowingAlert: $isShowingAlert, wordNote: myWordNote, word: word[num])
                .padding(.top)
...
struct LevelCheck: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            ...
            Button {
                // TODO: 모르겠어요 액션
                userStore.updateWordLevel(wordNote: wordNote, word: word, level: 0)
                ...
                .modifier(LevelCheckButton(backGroundColor: "Gray2"))
            }
            
            Button {
                // TODO: 애매해요 액션
                userStore.updateWordLevel(wordNote: wordNote, word: word, level: 1)
                if lastWordIndex != num {
                    isFlipped = false
                    totalScore += 0.25
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
                ...
                .modifier(LevelCheckButton(backGroundColor: "MainBlue"))
            }
            
            Button {
                // TODO: 외웠어요 액션
                userStore.updateWordLevel(wordNote: wordNote, word: word, level: 2)
                if lastWordIndex != num {
                    isFlipped = false
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
                        num += 1
                ...
                .modifier(LevelCheckButton(backGroundColor: "MainDarkBlue"))
            }
            ...
            Button("Ok") {
                Task{
                    ...
                    if !notiManager.isGranted{
                        ...
                        var localNotification = LocalNotification(identifier: UUID().uuidString, title: "MEMOrizing 암기 시간", body: "1번째 복습할 시간이에요~!", timeInterval: 1, repeats: false)
