...
//                    Spacer()
                    ...
                    if signUpPassWord != signUpPassWordCheck && !signUpPassWord.isEmpty && !signUpPassWordCheck.isEmpty {
                        Text("비밀번호가 동일하지 않습니다")
                            .modifier(CheckRightForm(textColor: "Red"))
                    } else  { // 1차 비밀번호 텍스트란에 내용이 있으며 1차와 2차가 같은 경우
                        Text(signUpPassWord == signUpPassWordCheck && !signUpPassWord.isEmpty ? "비밀번호가 동일합니다" : "")
                            .modifier(CheckRightForm(textColor: "MainBlue"))
                    }
                }
            } // VStack
            ...
            HStack{
                ...
                .sheet(isPresented: $isShownTerms) {
                    SignUpTermsView(termsAgreed: $termsAgreed)
                }
            ...
            Button {
                ...
                Text("회원가입 하기")
                    .modifier(nickName.isEmpty || signUpEmail.isEmpty || signUpPassWord.isEmpty || signUpPassWordCheck.isEmpty || signUpPassWord.count < 6 || signUpPassWord != signUpPassWordCheck ? CustomButtonStyle(backgroundColor: "Gray4") : CustomButtonStyle(backgroundColor: "MainBlue") )
                    .alert(" ", isPresented: $isShownAlert){
                        Button{
                            if termsAgreed{
                                userStore.emailAuthSignUp(email: signUpEmail, password: signUpPassWord, nickName: nickName)
                                dismiss()
                            }
                            signUpProcessing.toggle()
                        }label: {
            ...
            .disabled(nickName.isEmpty || signUpEmail.isEmpty || signUpPassWord.isEmpty || signUpPassWordCheck.isEmpty || signUpPassWord.count < 6 || signUpPassWord != signUpPassWordCheck)
